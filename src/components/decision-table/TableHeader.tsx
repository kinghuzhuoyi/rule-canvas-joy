      ...
      <div className={cn("absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20", "transition-all duration-200", isHovered ? "opacity-100 scale-100" : "opacity-0 scale-75 pointer-events-none")}>
        ...
        if (!open) setIsHovered(false);
            ...
            <PopoverContent className="p-0 w-auto" align="start">
              <VariableSelector onSelect={variable => {
            onAddInput?.(variable);
            setShowSelector(false);
            setIsHovered(false);
          }} onCancel={() => {
          ...
          if (open) {
            ...
            <PopoverContent className="p-3 min-w-[200px]" align="start">
              <div className="flex flex-col gap-2">
                ...
                {!column.isInput && <Select value={editType} onValueChange={v => setEditType(v as DataType)}>
                    <SelectTrigger className="h-8 text-sm">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
  ...
  return <div className="flex sticky top-8 z-20 bg-card border-b border-border min-w-max">
      ...
      <div className="flex bg-secondary/30">
        {inputColumns.map((col, index) => <React.Fragment key={col.id}>
            {/* 列前的分隔线 */}
            <ColumnDivider isInput={true} insertIndex={index} onAddInput={variable => onAddInputColumn(variable, index)} />
            {renderColumnHeader(col, canDeleteInput)}
          </React.Fragment>)}
        {/* 最后一列后的分隔线 */}
        <ColumnDivider isInput={true} insertIndex={inputColumns.length} onAddInput={variable => onAddInputColumn(variable, inputColumns.length)} />
      </div>
